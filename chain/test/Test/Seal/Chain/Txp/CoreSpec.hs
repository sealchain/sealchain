...
module Test.Seal.Chain.Txp.CoreSpec
       ( spec
       ) where
...
import           Seal.Chain.Txp (Tx (..), TxIn (..), TxOut (..), checkTx,
                     topsortTxs)
import           Seal.Core (mkCoin)
import           Seal.Core.Attributes (mkAttributes)
import           Seal.Crypto (hash, whData, withHash)
import           Seal.Util (_neHead)

import           Test.Seal.Chain.Txp.Arbitrary ()
import           Test.Seal.Util.QuickCheck.Arbitrary (sublistN)
