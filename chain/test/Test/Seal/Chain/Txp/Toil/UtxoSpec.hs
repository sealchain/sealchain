...
-- | Specification of Seal.Chain.Txp.Utxo

module Test.Seal.Chain.Txp.Toil.UtxoSpec
       ( spec
       ) where
...
import           Seal.Chain.Script (PlutusError (..), Script)
import           Seal.Chain.Script.Examples (alwaysSuccessValidator,
                     badIntRedeemer, goodIntRedeemer, goodIntRedeemerWithBlah,
...
import           Seal.Chain.Txp (ToilVerFailure (..), Tx (..), TxAux (..),
...
import           Seal.Core (addressHash, checkPubKeyAddress,
...
import           Test.Seal.Chain.Txp.Arbitrary (BadSigsTx (..),
                     DoubleInputTx (..), GoodTx (..), genGoodTxWithMagic)
import           Test.Seal.Crypto.Arbitrary (genProtocolMagicUniformWithRNM)
import           Test.Seal.Util.QuickCheck.Arbitrary (SmallGenerator (..),
                     nonrepeating, runGen)
import           Test.Seal.Util.QuickCheck.Property (qcIsLeft, qcIsRight)
