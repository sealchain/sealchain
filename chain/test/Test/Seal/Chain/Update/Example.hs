module Test.Seal.Chain.Update.Example
       ( exampleBlockVersion
...
import           Seal.Core (Coeff (..), CoinPortion (..), EpochIndex (..),
                     FlatSlotId, TxFeePolicy (..),
                     TxSizeLinear (..))
import           Seal.Crypto (ProtocolMagic (..), ProtocolMagicId (..),
                     RequiresNetworkMagic (..), hash)

import           Test.Seal.Core.ExampleHelpers (exampleAttributes,
