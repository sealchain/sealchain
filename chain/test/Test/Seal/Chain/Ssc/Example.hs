module Test.Seal.Chain.Ssc.Example
       ( exampleCommitment
...
import           Seal.Core (EpochIndex (..), addressHash)
import           Seal.Crypto (EncShare (..), ProtocolMagic (..),
...
import           Test.Seal.Core.ExampleHelpers (exampleEpochIndex,
...
-- Lifted from genSharedSecret in `Seal.Crypto.SecretSharing`.
