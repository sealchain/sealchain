...
module Test.Seal.Chain.Block.Arbitrary
       ( HeaderAndParams (..)
       , BlockHeaderList (..)
...
import           Seal.Chain.Block (HeaderHash, mkMainBlock, mkMainBlockExplicit)
...
import           Seal.Core (localSlotIndexMaxBound, localSlotIndexMinBound)
import qualified Seal.Core as Core
import           Seal.Core.Attributes (areAttributesKnown)
import           Seal.Crypto (ProtocolMagic, PublicKey, SecretKey, createPsk,
                     toPublic)

import           Test.Seal.Chain.Delegation.Arbitrary (genDlgPayload)
import           Test.Seal.Chain.Genesis.Dummy (dummyEpochSlots,
                     dummyGenesisHash)
import           Test.Seal.Chain.Ssc.Arbitrary (SscPayloadDependsOnSlot (..),
                     genSscPayload, genSscPayloadForSlot)
import           Test.Seal.Chain.Txp.Arbitrary (genTxPayload)
import           Test.Seal.Chain.Update.Arbitrary (genUpdatePayload)
import           Test.Seal.Core.Arbitrary (genSlotId)
...
-- 'Seal.Types.Blocks.Functions.verifyHeader', the blockheaders that may be
