...
-- | Specification of Seal.Chain.LrcPure (which is basically a pure
-- version of 'Seal.Chain.Lrc').

module Test.Seal.Chain.Lrc.FtsSpec
       ( spec
       ) where
...
import           Seal.Core (Coin, SharedSeed, StakeholderId, StakesList,
...
import           Test.Seal.Util.QuickCheck.Property (qcNotElem)
...
spec = do
  ...
  describe "Seal.Chain.LrcPure" $ do
