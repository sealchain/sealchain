...
module Seal.Chain.Contract.Lib where
...
#endif
#endif
import Seal.Contract.Typechecker
import Seal.Contract.Types.Typecheck
-- intentionally hidden unused functions to prevent lib functions from consuming gas
import Seal.Contract.Native.Internal hiding (defRNative,defGasRNative,defNative)
import qualified Seal.Contract.Native.Internal as Native
...
initLibState :: Loggers -> MPtreeDb -> IO LibState
initLibState loggers mpTree = do
  m <- newMVar (DbEnv mpTree persister
...
-- #endif
     ...
     ,defZRNative "json" json' (funType tTyValue [("exp",a)]) $
      ...
      "This is only needed for tests, as Seal.Contract values are automatically represented as JSON in API output. " <>
...
invokeEnv :: (MVar (DbEnv MPtreeDb) -> IO b) -> MVar LibState -> IO b
