...
import           Seal.Chain.Ssc.Base (secretToSharedSeed, verifyOpening,
                     vssThreshold)
import           Seal.Chain.Ssc.Commitment (Commitment (..), SignedCommitment,
                     getCommShares)
import           Seal.Chain.Ssc.CommitmentsMap (CommitmentsMap (..))
import           Seal.Chain.Ssc.Error.Seed (SscSeedError (..))
import           Seal.Chain.Ssc.Opening (Opening (..))
import           Seal.Chain.Ssc.OpeningsMap (OpeningsMap)
import           Seal.Chain.Ssc.SharesMap (SharesMap)
import           Seal.Core (Currency (..), SharedSeed, StakeholderId, addressHash)
import           Seal.Crypto (DecShare, Secret, VssPublicKey, recoverSecret,
                     verifySecret)
...
calculateSeed commitments' binVssKeys openings lShares richmen = do
    ...
    let getStake id = HM.lookupDefault (mkMoney 0) id richmen
        totalStake   = sumMoneys (fmap getStake (HM.keys richmen))
        secretsStake = sumMoneys (fmap getStake (HM.keys secrets))
    when (null secrets) $
        Left NoSecrets
    unless (secretsStake * 2 > totalStake) $
        Left $ NotEnoughParticipatingStake
                   ...
                   (unsafeIntegerToMoney secretsStake)
                   (unsafeIntegerToMoney totalStake)
