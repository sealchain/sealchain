...
module Seal.Chain.Ssc.Toss.Base
       (
         -- * Trivial functions
         getCommitment
       , hasCommitmentToss
...
import           Seal.Binary.Class (AsBinary, fromBinary)
import           Seal.Chain.Genesis as Genesis (Config)
import           Seal.Chain.Lrc.Types (RichmenSet, RichmenStakes)
import           Seal.Chain.Ssc.Base (verifyOpening, vssThreshold)
import           Seal.Chain.Ssc.Commitment (Commitment (..), SignedCommitment,
...
import           Seal.Chain.Ssc.OpeningsMap (OpeningsMap)
import           Seal.Chain.Ssc.Payload (SscPayload (..), spVss)
import           Seal.Chain.Ssc.SharesDistribution (SharesDistribution)
import           Seal.Chain.Ssc.SharesMap (InnerSharesMap, SharesMap)
import           Seal.Chain.Ssc.Toss.Class (MonadToss (..), MonadTossEnv (..),
                     MonadTossRead (..))
import           Seal.Chain.Ssc.VssCertificate (vcSigningKey, vcVssKey)
import           Seal.Chain.Ssc.VssCertificatesMap (VssCertificatesMap (..),
                     lookupVss, memberVss)
import           Seal.Chain.Update.BlockVersionData (bvdMpcThd)
import           Seal.Core (Currency (unsafeGetMoney), CoinPortion, EpochIndex (..), StakeholderId,
                     addressHash, coinPortionDenominator, getCoinPortion)
import           Seal.Crypto (DecShare, verifyDecShare, verifyEncShares)
import           Seal.Util.Util (getKeys, intords)
import           Seal.Util.Wlog (logWarning)
...
computeSharesDistrPure richmen threshold
  ...
  where
    ...
    coins :: [CoinUnsafe]
    coins = map (fromIntegral . unsafeGetMoney . snd) (HM.toList richmen)
