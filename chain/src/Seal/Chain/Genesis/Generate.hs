...
module Seal.Chain.Genesis.Generate
...
import           Seal.Binary.Class (asBinary, serialize')
import           Seal.Chain.Delegation.HeavyDlgIndex (HeavyDlgIndex (..),
                     ProxySKHeavy)
import           Seal.Chain.Ssc.VssCertificate (VssCertificate, mkVssCertificate)
import           Seal.Chain.Ssc.VssCertificatesMap (VssCertificatesMap,
                     mkVssCertificatesMap)
import           Seal.Core.Common (Address, Currency (..), Coin, 
                     IsBootstrapEraAddr (..), StakeholderId, 
                     addressHash, applyCoinPortionDown, deriveFirstHDAddress,
                     makePubKeyAddressBoot,)
import           Seal.Core.NetworkMagic (makeNetworkMagic)
import           Seal.Core.ProtocolConstants (ProtocolConstants, vssMaxTTL,
                     vssMinTTL)
import           Seal.Crypto (EncryptedSecretKey, ProtocolMagic, RedeemPublicKey,
...
import           Seal.Chain.Genesis.Delegation (GenesisDelegation (..),
                     mkGenesisDelegation)
import           Seal.Chain.Genesis.Initializer (FakeAvvmOptions (..),
...
generateGenesisData pm pc (GenesisInitializer{..}) realAvvmBalances = deterministic (serialize' giSeed) $ do
    ...
    let
        avvmSum = sumMoneys realAvvmMultiplied
        maxTnBalance =
            case moneyToInteger (maxBound @Coin) - avvmSum of
...
generateFakeAvvmGenesis FakeAvvmOptions{..} = do
    ...
    let oneBalance = mkMoney $ fromIntegral faoOneBalance
...
genTestnetDistribution TestnetBalanceOptions {..} testBalance =
  ...
  where
    ...
    richBalances =
        replicate (fromInteger richs) (unsafeIntegerToMoney oneRichmanBalance)
    poorBalances =
        replicate (fromInteger poors) (unsafeIntegerToMoney onePoorBalance)
