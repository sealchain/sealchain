module Seal.Chain.Genesis.NonAvvmBalances
...
import           Seal.Core.Common (Address, Coin, Currency (..), decodeTextAddress)
...
instance Aeson.ToJSON GenesisNonAvvmBalances where
    toJSON = Aeson.toJSON . convert . getGenesisNonAvvmBalances
      where
        ...
        f = bimap pretty (toInteger . unsafeGetMoney)
...
convertNonAvvmDataToBalances balances = GenesisNonAvvmBalances <$> balances'
  where
    ...
    balances' = HM.fromListWith unsafeAddMoney <$> traverse convert (HM.toList balances)
    convert :: (Text, Integer) -> m (Address, Coin)
    convert (txt, i) = do
        addr <- either throwError pure $ decodeTextAddress txt
        return (addr, unsafeIntegerToMoney i)
