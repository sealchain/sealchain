...
module Seal.Chain.Update.Poll.Class
       ( MonadPollRead (..)
       , MonadPoll (..)
...
import           Seal.Chain.Update.BlockVersionModifier (applyBVM)
...
import           Seal.Chain.Update.SoftwareVersion (NumSoftwareVersion,
                     SoftwareVersion (..))
import           Seal.Chain.Update.Vote (UpId, UpdateProposal (..))
import           Seal.Core (ChainDifficulty, Coin, EpochIndex, SlotId,
                     StakeholderId, addressHash)
import           Seal.Core.Slotting (SlottingData)
import           Seal.Crypto (hash)
import qualified Seal.Util.Modifier as MM
import           Seal.Util.Util (ether)
