...
module Seal.Chain.Update.Poll.Types
       (
         -- * Proposal state
...
import           Seal.Chain.Update.BlockVersionModifier
                     (BlockVersionModifier (..))
import           Seal.Chain.Update.SoftwareVersion (NumSoftwareVersion,
                     SoftwareVersion)
import           Seal.Chain.Update.Vote (UpId, UpdateProposal (..), UpdateVote)
import           Seal.Core (ChainDifficulty, Currency (..), Coin, StakeholderId)
import           Seal.Core.Slotting (EpochIndex, SlotId, SlottingData)
...
mkUProposalState upsSlot upsProposal =
    UndecidedProposalState
    { upsVotes = mempty , upsPositiveStake = mkMoney 0
    , upsNegativeStake = mkMoney 0
    , upsExtra = Nothing
    , ..
    }
