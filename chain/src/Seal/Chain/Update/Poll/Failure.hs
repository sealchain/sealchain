...
module Seal.Chain.Update.Poll.Failure
       ( PollVerFailure (..)
...
import           Seal.Chain.Update.BlockVersionModifier (BlockVersionModifier)
import           Seal.Chain.Update.SoftwareVersion (NumSoftwareVersion)
import           Seal.Chain.Update.Vote (UpAttributes, UpId)
import           Seal.Core (Currency (formatter), Coin, EpochIndex, StakeholderId)
import           Seal.Core.Reporting (MonadReporting, reportError)
...
instance Buildable PollVerFailure where
    ...
    build (PollSmallProposalStake threshold actual upId) =
        bprint ("proposal "%build%
                ...
                "(threshold is "%formatter%", proposal has "%formatter%")")
        upId threshold actual
    build (PollNotRichman id threshold stake) =
        bprint ("voter "%build%" is not richman (his stake is "%stext%", but"%
                " threshold is "%formatter%")")
        id (maybe "negligible" (sformat formatter) stake) threshold
