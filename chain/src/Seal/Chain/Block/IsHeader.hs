module Seal.Chain.Block.IsHeader
       ( IsHeader
       , IsGenesisHeader
       , IsMainHeader (..)
       ) where
...
import           Seal.Chain.Block.Header (BlockHeader, GenesisBlockHeader,
                     HasHeaderHash (..), MainBlockHeader, mainHeaderLeaderKey,
                     mainHeaderSlot)
import           Seal.Chain.Update.BlockVersion (HasBlockVersion (..))
import           Seal.Chain.Update.SoftwareVersion (HasSoftwareVersion (..))
import           Seal.Core.Common (HasDifficulty (..))
import           Seal.Core.Slotting (HasEpochIndex (..), HasEpochOrSlot (..),
                     SlotId (..))
import           Seal.Crypto (PublicKey)
import           Seal.Util.Some (Some, applySome, liftLensSome)
