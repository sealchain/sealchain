...
import           Seal.Core.Common (Coin, Currency (..), SharedSeed (..), 
                     SlotLeaders, StakeholderId)
import           Seal.Core.Slotting (LocalSlotIndex (..), SlotCount,
                     localSlotIndices)
import           Seal.Crypto (deterministic, randomNumber)

-- Note: The "Satoshi" is the smallest indivisble unit of a Bitcoin.
-- The currency of the Seal network is called "Ada" and its smallest
...
coinIndexOffset c = over _CoinIndex (+ unsafeGetMoney c)
...
-- currently hardcoded in 'Seal.Constants.epochSlots'). It is important to
...
followTheSatoshiM _ _ totalCoins
    | totalCoins == mkMoney 0 = error "followTheSatoshiM: nobody has any stake"
  ...
  where
    ...
    coinIndices = map (CoinIndex . fromInteger) . deterministic seed $
        replicateM (fromIntegral epochSlots)
            (randomNumber (moneyToInteger totalCoins))
...
followTheSatoshi epochSlots seed stakes
    | totalCoins > moneyToInteger (maxBound @Coin) =
        error $ sformat
  ...
  where
    totalCoins = sumMoneys $ map snd stakes
    totalCoinsCoin = mkMoney $ fromInteger totalCoins
