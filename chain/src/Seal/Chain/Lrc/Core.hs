...
import           Seal.Core.Common (Coin, Currency (..), StakeholderId)
...
findDelegationStakes isIssuer stakeResolver t = do
  ...
  where
    ...
    onItem (old, new) (delegate, issuers) = do
        sumIssuers <-
            foldM (\cr id -> (unsafeAddMoney cr) <$> safeGetStake id)
                  (mkMoney 0)
                  issuers
        isIss <- lift $ isIssuer delegate
        curStake <- if isIss then pure sumIssuers
                    else (unsafeAddMoney sumIssuers) <$> safeGetStake delegate
        let newRichmen =
    ...
    safeGetStake id = fromMaybe (mkMoney 0) <$> lift (stakeResolver id)
