...
module Seal.Infra.Communication.Types.Protocol
       ( checkInSpecs
       , Conversation (..)
       , convH
       , HandlerSpec (..)
...
import           Seal.Infra.Communication.BiP (BiP)
import           Seal.Infra.Network.Types (MsgType (..), NodeId (..),
                     NodeType (..), Origin (..))
import           Seal.Util.Util (cborError, toAesonError)
