...
module Seal.Infra.Network.Types
...
import qualified Seal.Infra.Network.DnsDomains as DnsDomains
...
initQueue NetworkConfig{..} loggerName mStore = liftIO $ do
    ...
    case mStore of
      ...
      Just store -> liftIO $ OQ.registerQueueMetrics (Just (toString sealNamespace)) oq store
