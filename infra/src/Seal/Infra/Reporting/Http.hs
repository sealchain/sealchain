...
module Seal.Infra.Reporting.Http
       ( sendReport
       , sendReportNodeImpl
       , reportNode
       ) where
...
import           Paths_seal_infra (version)
import           Seal.Crypto (ProtocolMagic (..), getProtocolMagic)
import           Seal.Core.Reporting.Report (BackendVersion (..), ReportInfo (..),
                     ReportType (..), Version (..))
...
sendReport pm compileInfo mLogFile reportType appName reportServerUri = do
  ...
  where
    ...
    mkReportInfo curTime =
        ...
        -- We are using version of 'seal-infra' here. We agreed
        -- that the version of 'seal' and it subpackages should
        -- be same.
...
sendReportNodeImpl logTrace protocolMagic compileInfo servers mLogFile reportType = do
    ...
    then onNoServers
        ...
        errors <-
            ...
            try . sendReport protocolMagic compileInfo mLogFile reportType "seal-node" . toString
