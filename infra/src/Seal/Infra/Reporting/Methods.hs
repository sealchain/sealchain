...
module Seal.Infra.Reporting.Methods
       ( Reporter (..)
...
import           Seal.Core.Exception (SealFatalError)
import           Seal.Core.Reporting (MonadReporting (..), Reporter (..),
                     noReporter, reportError, reportInfo)
import           Seal.DB.Error (DBError (..))
import           Seal.Util.Wlog (Severity (..), WithLogger, logMessage)
...
reportOrLog severity prefix exc =
    case tryCast @SealFatalError <|> tryCast @ErrorCall <|> tryCast @DBError of
