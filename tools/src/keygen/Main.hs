...
import           Seal.Binary (asBinary, serialize')
...
import           Seal.Chain.Ssc (mkVssCertificate, vcSigningKey)
import           Seal.Core (addressHash)
import           Seal.Crypto (EncryptedSecretKey (..), SecretKey (..),
                     VssKeyPair, fullPublicKeyF, hashHexF, noPassEncrypt,
                     redeemPkB64F, toPublic, toVssPublicKey)
import           Seal.Launcher (dumpGenesisData, withConfigurations)
import qualified Seal.Util.Log as Log
import           Seal.Util.Log.LoggerConfig (defaultInteractiveConfiguration)
import           Seal.Util.UserSecret (readUserSecret, takeUserSecret, usKeys,
                     usPrimKey, usVss, usWallet, writeUserSecretRelease,
                     wusRootKey)
import           Seal.Util.Wlog (Severity (Debug), WithLogger, logInfo,
