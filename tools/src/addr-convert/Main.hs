...
import           Seal.Core (makeRedeemAddress)
...
import           Seal.Crypto.Signing (fromAvvmPk)
...
getAddrConvertOptions = execParser programInfo
  where
    ...
    versionOption = infoOption
        ("seal-addr-convert-" <> showVersion version)
...
Command example:
  ...
  stack exec -- seal-addr-convert -a 2HF83bvYCTzoCbVta6t64W8rFEnvnkJbIUFoT5tOyoU= -p mainnet
