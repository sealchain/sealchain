...
import           Seal.Util (directory, ls, withTempDir)
...
getUpdateGenOptions = execParser programInfo
  where
    programInfo = info (helper <*> versionOption <*> optionsParser) $
        fullDesc <> progDesc ("")
                 <> header "Seal updates generator."
                 <> footerDoc usageExample

    versionOption = infoOption
        ("seal-genupdate-" <> showVersion version)
...
Command example:
  ...
  stack exec -- seal-genupdate --old /tmp/app-v000 --new /tmp/app-v001 -o /tmp/app-update.tar

Both directories must have equal file structure (e.g. they must contain the same
files in the same subdirectories correspondingly), otherwise 'seal-genupdate' will fail.

Please note that 'seal-genupdate' uses 'bsdiff' program, so make sure 'bsdiff' is available in the PATH.|]
