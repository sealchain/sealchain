...
optionsParser = do
    pathToBinDirectory <- strOption $
        long    "bin-dir" <>
        help    "Path to the directory with SEALCHAIN executable files." <>
        metavar "PATH"
...
getCLIDocsOptions pathToMarkdownFile = execParser programInfo
  where
    programInfo = info (helper <*> versionOption <*> optionsParser) $
        fullDesc <> progDesc "Generate Markdown chapter for sealdocs.com."
                 <> header "Tool to generate CLI-docs for Seal executable files."
                 <> footer ("Assumed that this program will run on CI. " <>
                            "Produced file '" <> pathToMarkdownFile <> "' will be " <>
                            "renamed in a chapter and pushed in sealdocs.com repository.")

    versionOption = infoOption
        ("seal-cli-docs-" <> showVersion version)
...
main = do
  ...
  where
    ...
    getExecutables binDir = filter sealExeOnly <$> listDirectory binDir
      where
        sealExeOnly exe = "seal-" `isPrefixOf` exe
...
SEALCHAIN CLI Options
----------------------

This guide describes all executable files that are used in SEALCHAIN and all corresponding CLI-options/parameters.
|]
