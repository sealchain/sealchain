module Seal.Tools.Dbgen.CLI where
...
import           Seal.Wallet.Web.ClientTypes.Types (AccountId, CAccountId (..))
import           Text.Read (readMaybe)

import           Seal.Tools.Dbgen.Types (Method (..))
...
data CLI = CLI
    ...
    -- ^ The path to a valid seal configuration.
...
instance ParseRecord CLI where
  parseRecord = CLI
              ...
              <*> (strOption (long "configPath" <> metavar "configuration-path"
                             <> help "A path to a valid seal configuration."
                                      ))
              <*> (strOption (long "configProf" <> metavar "configuration-profile"
                             <> help "A valid seal configuration profile to use."
                                      ))
