module Test.Seal.Util.WlogSpec
    ( spec)
where
...
import           Seal.Util.Log.LoggerConfig (defaultTestConfiguration,
                     lcLoggerTree, ltNamedSeverity)
import           Seal.Util.Wlog (Severity (..), WithLogger, addLoggerName,
...
someLogging :: IO ()
someLogging = do
    -- context: seal.testing
    usingLoggerName "testing" $ do
        testLog
        -- context: seal.testing.more
