...
import           Seal.Util.Log
import           Seal.Util.Log.Internal (getLinesLogged)
...
import           Seal.Util.Log.LogSafe (logDebugS, logErrorS, logInfoS,
      ...
      it "change minimum severity filter for a specific context" $
        monadicIO $ do
            let lc0 = defaultTestConfiguration Info
                newlt = lc0 ^. lcLoggerTree & ltNamedSeverity .~ HM.fromList [("seal.silent", Error)]
