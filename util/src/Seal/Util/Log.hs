...
import qualified Seal.Util.Log.Internal as Internal
import           Seal.Util.Log.LoggerConfig
import           Seal.Util.Log.LoggerName (LoggerName)
import           Seal.Util.Log.Scribes
import           Seal.Util.Log.Severity (Severity (..))
...
{- |
   * interactive tests
   ...
   >>> newlt <- return $ lc0 ^. lcLoggerTree & ltNamedSeverity .~ Data.HashMap.Strict.fromList [("seal-sl.silent", Error)]
