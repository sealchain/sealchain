{-|
Module:      Seal.Util.Concurrent.PriorityLock
...
import           Seal.Util.Queue (Q, dequeue, enqueue, queue)
...
unlockP (PriorityLock vstate) =
    atomically $ readTVar vstate >>= \case
        Unlocked -> error "Seal.Util.PriorityLock.unlockP: lock is already unlocked"
