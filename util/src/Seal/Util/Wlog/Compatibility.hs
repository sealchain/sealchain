...
module Seal.Util.Wlog.Compatibility
        (  -- * CanLog
           CanLog (..)
         , WithLogger
           -- * Pure logging
...
import           Seal.Util.Log (LoggingHandler, Severity (..))
import qualified Seal.Util.Log as Log
import qualified Seal.Util.Log.Internal as Internal
import           Seal.Util.Log.LoggerConfig (BackendKind (FileJsonBE),
...
--   Also, ToJSON a => KC.LogItem (see Seal.Util.Log).
