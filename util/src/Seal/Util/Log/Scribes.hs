...
import           Seal.Util.Log.LoggerConfig (NamedSeverity,
                     RotationParameters (..))
import           Seal.Util.Log.LoggerName (LoggerName)
import           Seal.Util.Log.Rotator (cleanupRotator, evalRotator,
                     initializeRotator)
import qualified Seal.Util.Log.Severity as Log (Severity (..))
