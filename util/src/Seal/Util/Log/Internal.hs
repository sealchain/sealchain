...
-- | internal definitions for "Seal.Util.Log"

module Seal.Util.Log.Internal
       ( newConfig
       , registerBackends
       , s2kname
       , s2knames
...
import           Paths_seal_util (version)
...
import           Seal.Util.Log.LoggerConfig (LoggerConfig (..))
...
registerBackends cfoKey lh scribes = do
    ...
    le0 <- K.initLogEnv (s2kname "seal") $ fromString $ (T.unpack cfoKey) <> ":" <> showVersion version
