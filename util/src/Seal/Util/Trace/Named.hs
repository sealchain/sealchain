...
import qualified Seal.Util.Log as Log
import           Seal.Util.Log.LoggerConfig (LogSecurityLevel (..))
import           Seal.Util.Log.LogSafe (SecuredText, logMCond, logMessageUnsafeP,
                     selectPublicLogs, selectSecretLogs)
import           Seal.Util.Trace (Trace (..), natTrace, traceWith)
import qualified Seal.Util.Trace.Unstructured as TrU (LogItem (..),
                     LogPrivacy (..))
...
logTrace' <- setupLogging "test" (Seal.Util.Log.LoggerConfig.defaultInteractiveConfiguration Log.Debug) "named"
...
logTrace' <- setupLogging "test" (Seal.Util.Log.LoggerConfig.jsonInteractiveConfiguration Log.Debug) "named"
