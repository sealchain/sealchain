...
module Seal.DB.Rocks.Functions
       (
...
import           Seal.DB.BatchOp (rocksWriteBatch)
import           Seal.DB.Class (DBIteratorClass (..), DBTag (..), IterType)
import           Seal.DB.Functions (processIterEntry)
import           Seal.DB.Rocks.Types (MonadRealDB, NodeDBs (..),
                     getDBByTag)
import           Seal.Mpt.MerklePatricia (MPDB (..), initializeBlank, emptyTriePtr)
import           Seal.RocksDB (DB (..))
import qualified Seal.Util.Concurrent.RWLock as RWL
...
openNodeDBs recreate fp = do
    ...
    _mpDB <- openRocksDB mpPath
    initializeBlank MPDB {rdb = _mpDB, stateRoot = emptyTriePtr}
