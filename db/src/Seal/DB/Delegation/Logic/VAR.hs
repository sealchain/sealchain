...
module Seal.DB.Delegation.Logic.VAR
       (
         dlgVerifyBlocks
       , dlgApplyBlocks
       , dlgRollbackBlocks
...
import           Seal.Chain.Block (Block, ComponentBlock (..), gbHeader,
                     headerHash, mainBlockDlgPayload, mainBlockSlot,
                     prevBlockL)
import           Seal.Chain.Delegation (CedeModifier (..), DlgBlund,
...
import           Seal.Chain.Lrc (RichmenSet)
import           Seal.Chain.Update (BlockVersionData)
import           Seal.Core (EpochIndex (..), StakeholderId, addressHash,
                     epochIndexL, siEpoch)
import           Seal.Core.Chrono (NE, NewestFirst (..), OldestFirst (..))
import           Seal.Crypto (ProxySecretKey (..), shortHashF)
import           Seal.DB (DBError (DBMalformed), MonadDBRead, SomeBatchOp (..))
import qualified Seal.DB as DB
import           Seal.DB.Delegation.Cede.Holders (MapCede, evalMapCede,
                     runDBCede)
import           Seal.DB.Delegation.Cede.Logic (CheckForCycle (..),
                     detectCycleOnAddition, dlgVerifyHeader, dlgVerifyPskHeavy,
                     getPskChain)
import qualified Seal.DB.Delegation.Core as GS
import           Seal.DB.Delegation.Logic.Common (DelegationError (..),
                     runDelegationStateAction)
import           Seal.DB.Delegation.Logic.Mempool (clearDlgMemPoolAction,
                     deleteFromDlgMemPool, processProxySKHeavyInternal)
import qualified Seal.DB.GState.Common as GS
import           Seal.DB.Lrc (HasLrcContext, getDlgRichmen)
import           Seal.Util (getKeys, _neHead)
