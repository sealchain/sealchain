...
module Seal.DB.Update.Poll.Logic.Softfork
       ( recordBlockIssuance
       , processGenesisBlock
       ) where
...
import           Seal.Chain.Update (BlockVersion, BlockVersionData (..),
                     ...
                     SoftforkRule (..))
import           Seal.Core (BlockCount, Currency (..), Coin, EpochIndex, 
                     SlotId (..), StakeholderId, crucialSlot)
import           Seal.DB.Update.Poll.Logic.Base (ConfirmedEpoch, CurEpoch,
...
calculateIssuersStake epoch BlockVersionState {..} =
    unsafeIntegerToMoney . sumMoneys <$>
