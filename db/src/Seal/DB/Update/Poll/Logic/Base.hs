...
module Seal.DB.Update.Poll.Logic.Base
       ( TotalPositive (..)
       , TotalNegative (..)
       , TotalSum (..)
       , mkTotPositive
...
import           Seal.Chain.Block (HeaderHash, IsMainHeader (..), headerHashG)
...
import           Seal.Core (Currency (..), Coin, CoinPortion (..), EpochIndex, SlotCount,
                     SlotId, TimeDiff (..), addressHash, applyCoinPortionUp,
                     coinPortionDenominator, difficultyL,
                     getCoinPortion, isBootstrapEra)
...
import           Seal.Crypto (PublicKey, hash, shortHashF)
import           Seal.Util.Wlog (WithLogger, logDebug, logNotice)
...
verifyNextBVMod upId epoch
  BlockVersionData { bvdMaxBlockSize = oldMBS
                   , bvdUnlockStakeEpoch = oldUnlockStakeEpoch
                   }
  BlockVersionModifier { bvmMaxBlockSize = newMBSM
...
mkTotPositive = TotalPositive . moneyToInteger

mkTotNegative :: Coin -> TotalNegative
mkTotNegative = TotalNegative . moneyToInteger

mkTotSum :: Coin -> TotalSum
mkTotSum = TotalSum . moneyToInteger
...
voteToUProposalState voter stake decision ups@UndecidedProposalState {..} = do
    ...
    let posStakeAfterRemove
            | oldPositive = upsPositiveStake `unsafeSubMoney` stake
            | otherwise = upsPositiveStake
        negStakeAfterRemove
            | oldNegative = upsNegativeStake `unsafeSubMoney` stake
            | otherwise = upsNegativeStake
    -- Then we recalculate stake adding stake of new vote.
        posStakeFinal
            | decision = posStakeAfterRemove `unsafeAddMoney` stake
            | otherwise = posStakeAfterRemove
        negStakeFinal
            | decision = negStakeAfterRemove
            | otherwise = negStakeAfterRemove `unsafeAddMoney` stake
...
putNewProposal slotOrHeader totalStake votesAndStakes up = insertActiveProposal ps
  where
    ...
    totalPositive = sumMoneys . map snd . filter (uvDecision . fst) $ votesAndStakes
    totalNegative = sumMoneys . map snd . filter (not . uvDecision . fst) $ votesAndStakes
    ...
    ups =
        ...
        , upsProposal = up
        , upsSlot = slotId
        , upsPositiveStake = unsafeIntegerToMoney totalPositive
        , upsNegativeStake = unsafeIntegerToMoney totalNegative
