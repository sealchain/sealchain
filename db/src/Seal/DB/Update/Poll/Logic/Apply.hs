...
module Seal.DB.Update.Poll.Logic.Apply
...
import           Seal.Core (BlockCount, ChainDifficulty (..), Currency (..), Coin, 
...
import           Seal.Crypto (hash, shortHashF)
import           Seal.DB.Update.Poll.Logic.Base (canBeAdoptedBV,
                     canCreateBlockBV, confirmBlockVersion, isDecided,
                     mkTotNegative, mkTotPositive, mkTotSum, putNewProposal,
                     voteToUProposalState)
import           Seal.DB.Update.Poll.Logic.Version (verifyAndApplyProposalBVS,
                     verifyBlockVersion, verifySoftwareVersion)
import           Seal.Util.Some (Some (..))
import           Seal.Util.Wlog (logDebug, logInfo, logNotice)
...
verifyProposalStake totalStake votesAndStakes upId = do
    thresholdPortion <- bvdUpdateProposalThd <$> getAdoptedBVData
    let threshold = applyCoinPortionUp thresholdPortion totalStake
    let thresholdInt = moneyToInteger threshold
    let votesSum =
            sumMoneys . map snd . filter (uvDecision . fst) $ votesAndStakes
    logDebug $
        sformat
