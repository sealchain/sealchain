...
module Seal.DB.Update.Poll.Logic.Normalize
       ( normalizePoll
       , refreshPoll
...
import           Seal.Core (Currency (..), Coin, EpochIndex, SlotId (siEpoch), 
                     addressHash, applyCoinPortionUp)
import           Seal.Crypto (PublicKey, hash)
import           Seal.DB.Update.Poll.Logic.Apply (verifyAndApplyProposal,
                     verifyAndApplyVoteDo)
import           Seal.Util.Util (getKeys, sortWithMDesc)
  ...
  where
    evaluatePropStake up =
        case votes ^. at (hash up) of
            Nothing         -> pure (mkMoney 0)
            Just votesForUP -> foldM step (mkMoney 0) (toList votesForUP)
    step accum vote
        | not (uvDecision vote) = pure accum
        | otherwise = unsafeAddMoney accum <$> evaluateVoteStake vote
    propToVotes up =
        let id = hash up
        in (id, ) <$> votes ^. at id
