...
module Seal.DB.Update.Logic.Global
       ( UpdateBlock
...
import           Seal.Chain.Block (ComponentBlock (..), headerHashG,
...
import           Seal.Core (StakeholderId, addressHash, epochIndexL)
import           Seal.Core.Chrono (NE, NewestFirst, OldestFirst)
import           Seal.Core.Exception (reportFatalError)
import           Seal.Core.Reporting (MonadReporting)
import           Seal.Core.Slotting (MonadSlotsData, SlottingData, slottingVar)
import qualified Seal.DB.BatchOp as DB
import qualified Seal.DB.Class as DB
import           Seal.DB.Lrc (HasLrcContext)
import           Seal.DB.Update.GState (UpdateOp (..))
import           Seal.DB.Update.Poll.DBPoll (DBPoll, runDBPoll)
import           Seal.DB.Update.Poll.Logic.Apply (verifyAndApplyUSPayload)
import           Seal.DB.Update.Poll.Logic.Base (canCreateBlockBV)
import           Seal.DB.Update.Poll.Logic.Rollback (rollbackUS)
import           Seal.DB.Update.Poll.Logic.Softfork (processGenesisBlock,
                     recordBlockIssuance)
import           Seal.Util.AssertMode (inAssertMode)
import qualified Seal.Util.Modifier as MM
import           Seal.Util.Wlog (WithLogger, modifyLoggerName)
