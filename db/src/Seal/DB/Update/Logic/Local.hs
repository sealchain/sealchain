...
module Seal.DB.Update.Logic.Local
       (
         -- * Proposals
         isProposalNeeded
...
import           Seal.Chain.Genesis as Genesis (Config, configBlockVersionData)
...
import           Seal.Core (SlotId (..), slotIdF)
import           Seal.Core.Reporting (MonadReporting)
import           Seal.Crypto (PublicKey, shortHashF)
import           Seal.DB.Class (MonadDBRead)
import qualified Seal.DB.GState.Common as DB
import           Seal.DB.GState.Lock (StateLock)
import           Seal.DB.Lrc (HasLrcContext)
import           Seal.DB.Update.Context (UpdateContext (..))
import qualified Seal.DB.Update.GState as DB
import           Seal.DB.Update.MemState (LocalVotes, MemPool (..),
                     MemState (..), MemVar (mvState), UpdateProposals,
                     addToMemPool, withUSLock)
import           Seal.DB.Update.Poll.DBPoll (runDBPoll)
import           Seal.DB.Update.Poll.Logic.Apply (verifyAndApplyUSPayload)
import           Seal.DB.Update.Poll.Logic.Normalize (filterProposalsByThd,
                     normalizePoll, refreshPoll)
import           Seal.Util.Util (HasLens (..), HasLens')
import           Seal.Util.Wlog (WithLogger, logWarning)
