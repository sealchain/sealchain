...
module Seal.DB.Txp.MemState.Types
       ( GenericTxpLocalData (..)
       , TxpLocalData
       ) where
...
import           Seal.Chain.Txp (MemPool, UndoMap, UtxoModifier, AccountModifier)
...
data GenericTxpLocalData extra = TxpLocalData
    ...
    , txpAccountModifier :: !(TVar AccountModifier)
    , txpPactModifier    :: !(TVar PactModifier)
