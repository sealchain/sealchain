...
module Seal.DB.Txp.Logic.Common
       ( buildUtxo
...
import           Seal.Chain.Txp (Tx (..), TxAux (..), TxIn (..), TxOutAux, Utxo,
                     UtxoModifier)
import           Seal.Crypto (hash)
import           Seal.DB.Class (MonadDBRead)
import           Seal.DB.Txp.Utxo (getTxOut)
...
buildUtxo ::
    ...
    -> [TxAux]
    -> m Utxo
buildUtxo = buildUtxoGeneric _txUtxoInputs
    ...
    toTxIns tx@UnsafeTx{..}
        ...
        | otherwise =
          ...
          in map (\i -> TxInUtxo txId i)
            [0 .. fromIntegral (length _txUtxoOutputs) - 1]
