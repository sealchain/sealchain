...
import           Seal.Chain.Txp (TxIn (..), TxOutAux (toaOut), Utxo,
                     addrBelongsToSet, genesisUtxo, txOutStake)
import           Seal.Core (Address, Currency (..), Coin)
import           Seal.DB (DBError (..), DBIteratorClass (..), DBTag (GStateDB),
                     IterType, MonadDB, MonadDBRead, RocksBatchOp (..),
                     dbIterSource, encodeWithKeyPrefix)
import           Seal.DB.GState.Common (gsGetBi, writeBatchGState)
...
sanityCheckUtxo genesisData expectedTotalStake = do
    ...
    calculatedTotalStake <-
        runConduitRes $ stakesSource .| CL.fold foldAdd (mkMoney @Coin 0)
    let fmt =
            ("Sum of stakes in Utxo differs from expected total stake (the former is "
             %formatter%", while the latter is "%formatter%")")
  ...
  where
    foldAdd acc stakes =
        acc `unsafeAddMoney` unsafeIntegerToMoney (sumMoneys stakes)
