...
module Seal.DB.Ssc.Logic.Local
       (
         sscGetLocalPayload
       , sscNormalize
...
import           Seal.Chain.Ssc (HasSscConfiguration, InnerSharesMap,
...
import           Seal.Core (BlockCount, EpochIndex, SlotId (..), StakeholderId,
                     epochIndexL, kEpochSlots)
import           Seal.Core.Slotting (MonadSlots (getCurrentSlot))
import           Seal.DB (MonadBlockDBRead, MonadDBRead,
                     MonadGState (gsAdoptedBVData))
import           Seal.DB.BlockIndex (getTipHeader)
import           Seal.DB.Lrc (HasLrcContext, getSscRichmen, tryGetSscRichmen)
import           Seal.Util.Wlog (WithLogger, launchNamedPureLog, logWarning)
