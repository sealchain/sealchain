...
-- system. On the contrary, modules like 'Seal.Chain.Block.VAR'
-- unite logic from multiple components into functions.

module Seal.DB.Block.Slog.Logic
       ( mustDataBeKnown
...
import           Seal.Chain.Block (Block, Blund, HasSlogGState, SlogUndo (..),
...
import           Seal.Chain.Genesis as Genesis (Config (..), configEpochSlots,
                     configK)
import           Seal.Chain.Update (BlockVersion (..), HasUpdateConfiguration,
                     lastKnownBlockVersion)
import           Seal.Core (BlockCount, FlatSlotId, ProtocolConstants,
                     difficultyL, epochIndexL, flattenSlotId, kEpochSlots,
                     pcBlkSecurityParam)
import           Seal.Core.Chrono (NE, NewestFirst (getNewestFirst),
...
import           Seal.Core.Slotting (MonadSlots, SlotId)
import           Seal.DB (SomeBatchOp (..))
import           Seal.DB.Block.BListener (MonadBListener (..))
import qualified Seal.DB.Block.GState.BlockExtra as GS
import           Seal.DB.Block.Load (putBlunds)
import           Seal.DB.Block.Slog.Context (slogGetLastSlots, slogPutLastSlots)
...
import           Seal.DB.Update.GState (getAdoptedBVFull)
