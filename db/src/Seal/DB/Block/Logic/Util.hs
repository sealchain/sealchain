...
module Seal.DB.Block.Logic.Util
       (
         -- * Common/Utils
         lcaWithMainChain
...
import           Seal.Chain.Block (BlockHeader, HasBlockConfiguration,
                     HasSlogGState, HeaderHash, fixedTimeCQ, headerHash,
                     prevBlockL)
import           Seal.Core (BlockCount, FlatSlotId, SlotCount, Timestamp (..),
                     difficultyL, flattenSlotId)
import           Seal.Core.Chrono (NE, NewestFirst (..), OldestFirst (..))
import           Seal.Core.Exception (reportFatalError)
import           Seal.Core.Slotting (MonadSlots (..), getCurrentSlotFlat,
                     slotFromTimestamp)
import           Seal.DB.Block.GState.BlockExtra (isBlockInMainChain)
import           Seal.DB.Block.Slog.Context (slogGetLastSlots)
import qualified Seal.DB.BlockIndex as DB
import           Seal.DB.Class (MonadBlockDBRead)
