...
module Seal.DB.Block.Logic.VAR
       ( verifyBlocksPrefix
...
import           Seal.Chain.Block (ApplyBlocksException (..), Block, Blund,
...
import           Seal.Core.Slotting (MonadSlots (getCurrentSlot), SlotId)
...
import           Seal.DB.Delegation (dlgVerifyBlocks)
import qualified Seal.DB.GState.Common as GS (getTip)
import           Seal.DB.Ssc (sscVerifyBlocks)
import           Seal.DB.Txp.Settings
                     (TxpGlobalSettings (TxpGlobalSettings, tgsVerifyBlocks))
import           Seal.DB.Update (getAdoptedBV, usVerifyBlocks)
import           Seal.Util (neZipWith4, spanSafe, _neHead)
...
verifyBlocksPrefix genesisConfig currentSlot blocks = runExceptT $ do
    ...
    -- 'slogVerifyBlocks' uses 'Seal.Chain.Block.Pure.verifyBlocks' which does
