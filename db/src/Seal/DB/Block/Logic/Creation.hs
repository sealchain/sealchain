...
module Seal.DB.Block.Logic.Creation
...
import           Seal.Chain.Block (BlockHeader (..), GenesisBlock,
                     HasSlogGState (..), HeaderHash, MainBlock, MainBody,
                     headerHash, mkGenesisBlock, mkMainBlock)
import qualified Seal.Chain.Block as BC
import           Seal.Chain.Delegation (DelegationVar, DlgPayload (..),
                     ProxySKBlockInfo)
import           Seal.Chain.Genesis as Genesis (Config (..),
                     configBlkSecurityParam, configBlockVersionData,
                     configEpochSlots)
import           Seal.Chain.Ssc (MonadSscMem, SscPayload, defaultSscPayload,
                     stripSscPayload)
import           Seal.Chain.Txp (TxAux (..), TxpConfiguration, emptyTxPayload,
                     mkTxPayload)
import           Seal.Chain.Update (HasUpdateConfiguration, UpdatePayload (..),
                     curSoftwareVersion, lastKnownBlockVersion)
import           Seal.Core (BlockCount, EpochIndex, EpochOrSlot (..),
                     SlotId (..), epochIndexL, flattenSlotId, getEpochOrSlot,
                     kChainQualityThreshold, kEpochSlots,
                     localSlotIndexMinBound)
import           Seal.Core.Context (HasPrimaryKey, getOurSecretKey)
import           Seal.Core.Exception (assertionFailed, reportFatalError)
import           Seal.Core.JsonLog (CanJsonLog (..))
import           Seal.Core.JsonLog.LogEvents (MemPoolModifyReason (..))
import           Seal.Core.Reporting (HasMisbehaviorMetrics, reportError)
import           Seal.Core.Slotting (MonadSlots (getCurrentSlot))
import           Seal.Core.Util.LogSafe (logInfoS)
import           Seal.Crypto (SecretKey)
import           Seal.DB.Block.Logic.Internal (MonadBlockApply,
                     applyBlocksUnsafe, normalizeMempool)
import           Seal.DB.Block.Logic.Util (calcChainQualityM)
import           Seal.DB.Block.Logic.VAR (verifyBlocksPrefix)
import           Seal.DB.Block.Lrc (LrcModeFull, lrcSingleShot)
import           Seal.DB.Block.Slog.Logic (ShouldCallBListener (..))
import qualified Seal.DB.BlockIndex as DB
import           Seal.DB.Class (MonadDBRead)
import           Seal.DB.Delegation (clearDlgMemPool, getDlgMempool)
import           Seal.DB.GState.Lock (Priority (..), StateLock, StateLockMetrics,
                     modifyStateLock)
import           Seal.DB.Lrc (HasLrcContext, lrcActionOnEpochReason)
import qualified Seal.DB.Lrc as LrcDB
import           Seal.DB.Ssc (sscGetLocalPayload, sscResetLocal)
import           Seal.DB.Txp (MempoolExt, MonadTxpLocal (..), MonadTxpMem,
                     clearTxpMemPool, txGetPayload, withTxpLocalData, getTxpTip)
import           Seal.DB.Txp.Pact (getPactStateRoot)
import           Seal.DB.Update (UpdateContext, clearUSMemPool, getMaxBlockSize,
                     usCanCreateBlock, usPreparePayload)
import           Seal.Mpt.MerklePatricia (getMPDB)
import           Seal.Contract.Persist.MPTree (initMPtree)
import           Seal.Util (_neHead)
...
applyCreatedBlock genesisConfig txpConfig pske createdBlock = applyCreatedBlockDo False createdBlock
  where
    ...
    clearMempools = do
        ...
        pactStateRoot <- getPactStateRoot txpTip
        pactMPDB <- getMPDB pactStateRoot
        let pactModifier = initMPtree pactMPDB

        withTxpLocalData (clearTxpMemPool pactModifier)
