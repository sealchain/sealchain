...
module Seal.DB.Block.GState.BlockExtra
       ( resolveForwardLink
       , isBlockInMainChain
       , isHeaderHashInMainChain
       , getLastSlots
...
import           Seal.Chain.Block (Block, BlockHeader, HasHeaderHash, HeaderHash,
                     LastBlkSlots, headerHash, noLastBlkSlots)
import           Seal.Chain.Genesis (GenesisHash (..))
import           Seal.Core (FlatSlotId, SlotCount, slotIdF, unflattenSlotId)
import           Seal.Core.Chrono (OldestFirst (..))
import           Seal.Crypto (shortHashF)
import           Seal.DB (DBError (..), MonadDB, MonadDBRead (..),
                     RocksBatchOp (..), getHeader)
import           Seal.DB.Class (MonadBlockDBRead, SerializedBlock, getBlock)
...
isHeaderHashInMainChain
    :: (MonadDBRead m)
    => HeaderHash -> m Bool
isHeaderHashInMainChain hh =
    maybe False (\() -> True) <$> gsGetBi (mainChainKey hh)
