module Test.Seal.Core.Gen
       (
        -- Seal.Core.Common Generators
          genAddrAttributes
        , genAddress
        , genAddrSpendingData
        ...
        -- Seal.Core.Attributes Generators
        , genAttributes

        -- Seal.Merkle Generators
        , genMerkleRoot
        , genMerkleTree
        , genHashRaw
...
import           Test.Seal.Crypto.Gen (genAbstractHash, genHDAddressPayload,
                     genPublicKey, genRedeemPublicKey)
import           Test.Seal.Util.Gen (genHashMap)
...
genAddrAttributes = AddrAttributes <$> hap <*> genAddrStakeDistribution <*> ac
  where
    hap = Gen.maybe genHDAddressPayload
    ac  = Gen.maybe genAddrCategory

genAddrCategory :: Gen AddrCategory
genAddrCategory = Gen.choice [ pure ACMobile
                             , pure ACUserAccount
                             , pure ACContractAccount
                             , ACUnknown <$> genWord8
                             ]
...
genAddrStakeDistribution = Gen.choice gens
  where
    ...
    -- Lifted from `Seal.Arbitrary.Core`. There are very particular constraints
...
genCoeff = do
    ...
    -- in Seal correspond to less than 1 ADA.
...
----------------------------------------------------------------------------
-- Seal.Core.ProtocolConstants Generators
----------------------------------------------------------------------------
