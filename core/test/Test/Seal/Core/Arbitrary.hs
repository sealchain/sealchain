...
module Test.Seal.Core.Arbitrary
       ( CoinPairOverflowSum (..)
       , CoinPairOverflowSub (..)
       , CoinPairOverflowMul (..)
...
import           Seal.Core (AddrCategory (..), AddrAttributes (..), AddrSpendingData (..),
                     AddrStakeDistribution (..), AddrType (..), Address (..),
                     Address' (..), BlockCount (..), ChainDifficulty (..),
                     Coeff (..), Coin (..), CoinPortion (..), EpochIndex (..),
                     EpochOrSlot (..), LocalSlotIndex (..), GoldCoin (..),
                     SharedSeed (..), SlotCount (..), SlotId (..), GoldDollar (..),
                     StakeholderId, TimeDiff (..), Timestamp (..), CoinGroup (..),
                     TxFeePolicy (..), TxSizeLinear (..), Cmd (..),
                     coinPortionDenominator,
                     localSlotIndexMaxBound, localSlotIndexMinBound,
                     makeAddress, mkLocalSlotIndex, mkMultiKeyDistr,
                     unsafeCoinPortionFromDouble)
import           Seal.Core.Attributes (Attributes (..), UnparsedFields (..))
import           Seal.Core.Constants (sharedSeedLength)
import           Seal.Core.Merkle (MerkleTree, mkMerkleTree)
import           Seal.Core.NetworkMagic (NetworkMagic (..))
import           Seal.Core.ProtocolConstants (pcEpochSlots)
import           Seal.Util.Util (leftToPanic)

import           Test.Seal.Core.Dummy (dummyProtocolConstants)
...
'Seal.Util.Util.dumpSplices' can be used to verify this.'
-}
...
instance Arbitrary AddrCategory where
    arbitrary = genericArbitrary
    shrink = genericShrink
...
genAddress :: Maybe AddrCategory -> Gen Address
genAddress ac = makeAddress <$> arbitrary <*> genAddrAttr
  where
    genAddrAttr = AddrAttributes <$> arbitrary <*> arbitrary <*> pure ac
...
instance Arbitrary GoldCoin where
    arbitrary = unSafetoGoldCoin <$> choose (1, 1700)
    shrink = genericShrink

instance Arbitrary GoldDollar where
    arbitrary = unSafetoGoldDollar <$> choose (1, 17000)
    shrink = genericShrink

instance Arbitrary CoinGroup where
    arbitrary = CoinGroup
        <$> arbitrary
        <*> arbitrary
        <*> arbitrary
...
----------------------------------------------------------------------------
-- Arbitrary types from 'Seal.Core.Fee'
----------------------------------------------------------------------------
...
-- | Makes a 'Coin' but is _|_ if that coin exceeds 'maxCoinVal'.
-- You can also use 'checkCoin' to do that check.
unSafetoGoldCoin :: Word64 -> GoldCoin
unSafetoGoldCoin g = GoldCoin g
{-# INLINE unSafetoGoldCoin #-}

unSafetoGoldDollar :: Word64 -> GoldDollar
unSafetoGoldDollar g = GoldDollar g
{-# INLINE unSafetoGoldDollar #-}
