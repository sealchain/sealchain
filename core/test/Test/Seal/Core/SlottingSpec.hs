-- | Specification of Seal.Core.Slotting.

module Test.Seal.Core.SlottingSpec
       ( spec
       ) where
...
import           Seal.Core (EpochOrSlot, SlotId (..), epochOrSlotFromEnum,
                     epochOrSlotMaxBound, epochOrSlotMinBound, epochOrSlotPred,
                     epochOrSlotSucc, epochOrSlotToEnum, flattenSlotId,
                     unflattenSlotId)

import           Test.Seal.Core.Arbitrary (EoSToIntOverflow (..),
                     UnreasonableEoS (..))
import           Test.Seal.Core.Dummy (dummyEpochSlots)
import           Test.Seal.Util.QuickCheck.Property (shouldThrowException, (.=.))
