...
-- | Seal.Util specification

module Test.Seal.Core.ChronoSpec
       ( spec
       ) where
...
import           Seal.Core.Chrono (Chrono (..), NE, NewestFirst (..),
                     OldestFirst (..))

import           Test.Hspec (Expectation, Spec, describe, shouldBe)
import           Test.Hspec.QuickCheck (prop)

import           Test.Seal.Binary.Helpers (U, binaryTest)
import           Test.Seal.Core.Chrono ()
import           Test.Seal.Util.Modifier ()
import           Test.Seal.Util.QuickCheck.Property ((.=.))
