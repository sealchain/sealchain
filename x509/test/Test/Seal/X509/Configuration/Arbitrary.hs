...
module Test.Seal.X509.Configuration.Arbitrary
    ( Invalid(..)
    , Unknown(..)
    , AltNames(..)
    ) where
...
import           Seal.X509.Configuration (CertConfiguration (..),
...
instance Arbitrary CertConfiguration where
    arbitrary = CertConfiguration
        <$> elements ["SEAL", "SEALCHAIN", "SEAL Foundation"]
        <*> elements ["PC Wallet", "Mobile Wallet", "Prometheus", "Root CA"]
        <*> arbitraryPositive
