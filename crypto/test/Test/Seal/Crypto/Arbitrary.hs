...
module Test.Seal.Crypto.Arbitrary
       ( SharedSecrets (..)
       , genSignature
       , genSignatureEncoded
       , genRedeemSignature
...
import           Seal.Crypto.Signing.Redeem (RedeemPublicKey, RedeemSecretKey,
                     RedeemSignature, redeemKeyGen, redeemSign)

import           Seal.Util.Orphans ()

import           Test.Seal.Crypto.Arbitrary.Unsafe ()
import           Test.Seal.Crypto.Dummy (dummyProtocolMagic)
import           Test.Seal.Util.Orphans ()
import           Test.Seal.Util.QuickCheck.Arbitrary (Nonrepeating (..),
...
-- 'arbitrary' and then use 'Seal.Crypto.toPublic' to get the corresponding
-- public key.
