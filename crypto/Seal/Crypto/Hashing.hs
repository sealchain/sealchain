...
import           Seal.Binary.Class (Bi (..), Raw, withWordSize)
import qualified Seal.Binary.Class as Bi
import           Seal.Binary.SafeCopy (getCopyBi, putCopyBi)
import           Seal.Util.Util (parseJSONWithRead, toAesonError, toCborError)
...
hashDigestSize' = hashDigestSize @algo
    (error "Seal.Crypto.Hashing.hashDigestSize': HashAlgorithm value is evaluated!")
