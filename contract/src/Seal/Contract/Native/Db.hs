...
-- |
-- Module      :  Seal.Contract.Native.Db
...
read' g0 i [table@TTable {},TLitString key,TObject ps _ _] = do
  guardTable i table
  mrow <- readRow (_faInfo i) (userTable table) (RowKey key)
  case mrow of
    Nothing -> fmap (g0, ) $ return $ TObject ps (_tTableType table) def
    Just cs -> do
      g <- gasPostRead i g0 cs
      fmap (g, ) $ return $ columnsToObject (_tTableType table) cs
...
select :: NativeFun e
select i as@[tbl', cols'@TList{}, app@TApp{}] = do
  cols <- reduce cols' >>= colsToList (argsError' i as)
  reduce tbl' >>= select' i as (Just cols) (Just app)
select i as@[tbl', cols'@TList{}] = do
  cols <- reduce cols' >>= colsToList (argsError' i as)
  reduce tbl' >>= select' i as (Just cols) Nothing
select i as@[tbl', app@TApp{}] = reduce tbl' >>= select' i as Nothing (Just app)
select i as@[tbl'] = reduce tbl' >>= select' i as Nothing Nothing
select i as = argsError' i as


select' :: FunApp -> [Term Ref] -> Maybe [(Info,ColumnId)] ->
  Maybe (Term Ref) -> Term Name -> Eval e (Gas,Term Name)
select' i _ cols' app tbl@TTable{} = do
        ...
        Just row -> do
          ...
          fmap (g,) $ case result of
            (TLiteral (LBool include) _)
              ...
              | otherwise -> return rs
            t -> evalError fi $ "select: filter returned non-boolean value: " ++ show t
...
guardTable i TTable {..} = do
  ...
  case r of
    ...
    _ -> do
      m <- getModule (_faInfo i) _tModule
      enforceKeySetName (_faInfo i) (_mName m) (_mKeySet m)
