...
{-# LANGUAGE LambdaCase #-}
-- |
-- Module      :  Seal.Contract.Native.Ops
...
defLogic n bop shortC =
 ...
 where
  fun :: NativeFun e
  fun i as = gasUnreduced i as $ catch (logicFun' as) (\(_ :: SomeException) -> (reduce (last as) >>= liftFun' (init as))) >>= \case 
      Just a -> return a
      _      -> argsError' i as
  logicFun' :: [Term Ref] -> Eval e (Maybe (Term Name))
  ...
  liftFun' :: [Term Ref] -> Term Name -> Eval e (Maybe (Term Name))
...
-- liftLogic ::
--      NativeDefName -> (Bool -> Bool -> Bool) -> Text -> Bool -> NativeDef
-- liftLogic n bop desc shortCircuit =
--   defNative n fun
--     (funType tTyBool [("a", logicLam r), ("b", logicLam r), ("value", r)])
--     ("Apply logical '" <> desc <>
--      "' to the results of applying VALUE to A and B, with short-circuit. " <>
--      "`(" <> asString n <> " (> 20) (> 10) 15)`")
--   where
--     r = mkTyVar "r" []
--     fun i as@[a@TApp {}, b@TApp {}, v'] =
--       gasUnreduced i as $
--       reduce v' >>= \v -> do
--         ar <- apply' a [v]
--         case ar of
