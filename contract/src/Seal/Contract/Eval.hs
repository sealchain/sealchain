...
-- |
-- Module      :  Seal.Contract.Eval
-- Copyright   :  (C) 2016 Stuart Popejoy
-- License     :  BSD-style (see the file LICENSE)
-- Maintainer  :  Stuart Popejoy <stuart@kadena.io>
--
-- Seal.Contract interpreter.
--
...
enforceKeySetName ::  Info -> ModuleName -> KeySetName ->  Eval e ()
enforceKeySetName mi (ModuleName modName) mksn@(KeySetName ksName) = do
  --如果是系统合约，则从数据库中读取keyset，用户合约不需要
  ks <- if modName == "Token" then do
           ks' <- maybe (evalError mi $ "No such keyset: " ++ show mksn) return =<< readRow mi KeySets mksn
           return ks'
        else do
           let kName = Name "keys-any" mi
           let sName = T.unpack ksName
           let pk = PublicKey (BSC.pack sName)
           let ks' = KeySet [pk] kName
           return ks'
...
-- eval (TNs ns i) = topLevelCall i "ns" GUser $ \g ->
--   return (g,tStr $ pack $ "Namespace " ++ show ns)
eval (TModule m@Module{..} bod i) =
  topLevelCall i "contract" (GModule m) $ \g0 -> do
    ...
    case oldM of
      ...
      Just om ->
        case om of
          Module{..} -> enforceKeySetName i _mName _mKeySet
    ...
    enforceKeySetName i _mName _mKeySet
...
reduce TSchema {..} = do
  TSchema _tSchemaName _tModule _tMeta <$> traverse (traverse reduce) _tFields <*> pure _tInfo
reduce TEvent {..} = do
  TEvent _tEventName _tModule _tMeta <$> traverse (traverse reduce) _tFields <*> pure _tInfo
reduce TTable {..} = do
  TTable _tTableName _tModule _tHash <$> mapM reduce _tTableType <*> pure _tMeta <*> pure _tInfo
