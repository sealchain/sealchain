...
import Seal.Contract.Types.Util (AsString(..))
import Prelude
...
-- | Represent Seal.Contract 'Term' values that can be stored in a database.
data Persistable =
    PLiteral Literal |
    PKeySet KeySet |
    PValue Value
...
newtype ColumnId = ColumnId Text
    deriving (Eq,Ord,IsString,AsString,ToJSON,FromJSON,Default)
instance Show ColumnId where show (ColumnId s) = show s
instance ToTerm ColumnId where toTerm (ColumnId s) = tLit $ LKeyword s
...
-- | Fun-record type for Seal.Contract back-ends.
