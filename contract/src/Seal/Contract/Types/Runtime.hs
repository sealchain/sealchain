...
{-# LANGUAGE GADTs #-}
-- |
-- Module      :  Seal.Contract.Types.Runtime
...
module Seal.Contract.Types.Runtime
 ( 
   ...
   throwDbError,throwEither,throwErr,
   PactId(..),
  ...
  updateRefStore,
   EntityName(..),
   EvalEnv(..),eeRefStore,eeMsgSigs,eeMsgBody,eeTxId,eeEntity,eePactStep,eePactDbVar,eePactDb,eePurity,eeHash,eeGasEnv,eeAccountMPDB,
   ...
   EvalState(..),evalRefs,evalCallStack,evalPactExec,evalGas,evalAccountMM,
   ...
   module Seal.Contract.Types.Lang,
   module Seal.Contract.Types.Util,
   module Seal.Contract.Types.Persistence,
...
import Seal.Contract.Types.Gas
import Seal.Contract.Types.Lang
import Seal.Contract.Types.Orphans ()
...
data EvalState = EvalState {
    ...
    , _evalCallStack :: ![StackFrame]
      -- | Seal.Contract execution trace, if any
    , _evalPactExec :: !(Maybe PactExec)
      -- | Gas tally
    , _evalGas :: Gas
    , _evalAccountMM :: MM.MapModifier Account AccountState
    } deriving (Show)
makeLenses ''EvalState
instance Default EvalState where def = EvalState def def def 0 mempty
...
instance Monoid a => Monoid (Eval e a) where
  mempty = pure mempty
  mappend = (<>)  --todo code review? -Gyoung 2018.11.28
