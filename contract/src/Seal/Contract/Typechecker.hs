...
-- |
-- Module      :  Seal.Contract.Typechecker
-- Copyright   :  (C) 2017 Stuart Popejoy
-- License     :  BSD-style (see the file LICENSE)
-- Maintainer  :  Stuart Popejoy <stuart@kadena.io>
--
-- Stylized static typechecker for Seal.Contract, operates on
...
-- function.
--
-- Seal.Contract's support for overloaded native functions is one reason
-- a more traditional HM approach is not used.
--
-- TODO: handle lazy functions better (map, filter etc).
--

module Seal.Contract.Typechecker where
...
import Seal.Contract.Types.Native
import Universum ((<>))
import Prelude
...
walkAST :: (MonadFail m) => Visitor m n -> AST n -> m (AST n)
...
toAST TNamespace{..} = die _tInfo "Namespace not supported"
