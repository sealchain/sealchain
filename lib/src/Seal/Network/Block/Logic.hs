...
-- nodes. Also see "Seal.Chain.Block.Network.Retrieval" for retrieval worker
-- loop logic.
module Seal.Network.Block.Logic
       (
...
import           Seal.Chain.Block (ApplyBlocksException, Block, BlockHeader,
...
import           Seal.Core (SlotCount, isMoreDifficult)
...
import           Seal.DB.Block (ClassifyHeaderRes (..), classifyNewHeader,
                     lcaWithMainChain, verifyAndApplyBlocks)
import qualified Seal.DB.Block as L
import qualified Seal.DB.Block as DB
import           Seal.DB.GState.Lock (Priority (..), modifyStateLock)
import           Seal.Infra.Communication.Protocol (NodeId)
import           Seal.Infra.Diffusion.Types (Diffusion)
import qualified Seal.Infra.Diffusion.Types as Diffusion
import           Seal.Infra.Recovery.Info (recoveryInProgress)
...
import           Seal.Util (buildListBounds, multilineBounds, _neLast)
...
-- in seal-block; it should just use the Diffusion and Logic interfaces.
