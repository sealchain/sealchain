...
module Seal.Network.Update.Download
       ( installerHash
       , downloadUpdate
       ) where
...
import           Seal.Crypto (Hash, castHash, hash)
import           Seal.DB.Update (UpdateContext (..), isUpdateInstalled)
import           Seal.Infra.Reporting (reportOrLogW)
import           Seal.Listener.Update (UpdateMode)
import           Seal.Util.Concurrent (withMVar)
import           Seal.Util.Util (HasLens (..), (<//>))
import           Seal.Util.Wlog (WithLogger, logDebug, logInfo, logWarning)
