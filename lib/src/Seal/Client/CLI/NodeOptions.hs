...
-- | Command line options of seal node.

module Seal.Client.CLI.NodeOptions
       ( CommonNodeArgs (..)
       , SimpleNodeArgs (..)
       , NodeArgs (..)
...
import           Paths_seal (version)

import           Seal.Client.CLI.Options (CommonArgs (..), commonArgsParser,
                     optionalJSONPath)
import           Seal.Core.NetworkAddress (NetworkAddress)
import           Seal.Infra.HealthCheck.Route53 (route53HealthCheckOption)
import           Seal.Infra.InjectFail (FInjectsSpec, parseFInjectsSpec)
import           Seal.Infra.Network.CLI (NetworkConfigOpts, networkConfigOption)
import           Seal.Infra.Statistics (EkgParams, StatsdParams, ekgParamsOption,
                     statsdParamsOption)
import           Seal.Util.CompileInfo (CompileTimeInfo (..), HasCompileInfo,
                     compileInfo)
...
commonNodeArgsParser = do
    ...
    cnaAssetLockPath <- optional $ strOption $
        ...
        help    "Path to list of assetLocked source addresses. Funds at these \
                ...
                \while Seal is centrally mined/minted. Addresses should be listed \
    ...
    cnaFInjectsSpec <- parseFInjectsSpec
        -- The fault injection CLI switches are generated in infra/src/Seal/Infra/InjectFail.hs
...
getSimpleNodeOptions = execParser programInfo
  where
    programInfo = info (helper <*> versionOption <*> simpleNodeArgsParser) $
        fullDesc <> progDesc "Seal main server node."
                 <> header "Seal node."
                 <> footerDoc usageExample

    versionOption = infoOption
        ("seal-node-" <> showVersion version <>
