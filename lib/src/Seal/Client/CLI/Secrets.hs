...
module Seal.Client.CLI.Secrets
       ( prepareUserSecret
       ) where
...
import           Seal.Chain.Genesis (GeneratedSecrets (..), RichSecrets (..))
import           Seal.Crypto (SecretKey, VssKeyPair, keyGen, runSecureRandom,
                     vssKeyGen)
import           Seal.Util.UserSecret (UserSecret, usPrimKey, usVss,
                     writeUserSecret)
import           Seal.Util.Wlog (WithLogger, logInfo)

import           Seal.Client.CLI.NodeOptions (CommonNodeArgs (..))
