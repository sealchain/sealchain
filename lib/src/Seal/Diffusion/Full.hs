...
import           Seal.Chain.Ssc (InnerSharesMap, MCCommitment (..),
                     MCOpening (..), MCShares (..), MCVssCertificate (..),
                     Opening, SignedCommitment, VssCertificate)
import           Seal.Chain.Txp (TxAux)
import           Seal.Chain.Update (BlockVersion, BlockVersionData (..), UpId,
                     UpdateProposal, UpdateVote)
import           Seal.Communication (EnqueueMsg, HandlerSpecs, InSpecs (..),
...
import           Seal.Core (ProtocolConstants (..), StakeholderId)
import           Seal.Core.Chrono (OldestFirst)
import           Seal.Core.Metrics.Constants (withSealNamespace)
import           Seal.Crypto.Configuration (ProtocolMagic (..), getProtocolMagic)
import qualified Seal.Diffusion.Full.Block as Diffusion.Block
import qualified Seal.Diffusion.Full.Delegation as Diffusion.Delegation
import qualified Seal.Diffusion.Full.Ssc as Diffusion.Ssc
import qualified Seal.Diffusion.Full.Txp as Diffusion.Txp
import qualified Seal.Diffusion.Full.Update as Diffusion.Update
import           Seal.Infra.Communication.Relay.Logic (invReqDataFlowTK)
import           Seal.Infra.DHT.Real (KademliaDHTInstance (..),
                     KademliaParams (..), kademliaJoinNetworkNoThrow,
                     kademliaJoinNetworkRetry, startDHTInstance,
                     stopDHTInstance)
import           Seal.Infra.Diffusion.Subscription.Common (subscriptionListeners)
import           Seal.Infra.Diffusion.Subscription.Dht (dhtSubscriptionWorker)
import           Seal.Infra.Diffusion.Subscription.Dns (dnsSubscriptionWorker)
import           Seal.Infra.Diffusion.Subscription.Status (SubscriptionStates,
                     emptySubscriptionStates)
import           Seal.Infra.Diffusion.Transport.TCP (bracketTransportTCP)
import           Seal.Infra.Diffusion.Types (Diffusion (..),
                     DiffusionHealth (..), DiffusionLayer (..))
import           Seal.Infra.Network.Types (Bucket (..), NetworkConfig (..),
...
import           Seal.Util.OutboundQueue (EnqueuedConversation (..))
import           Seal.Util.Timer (Timer, newTimer)
import           Seal.Util.Trace (Severity (Error), Trace)
...
diffusionLayerFullExposeInternals
    ...
    -> IO HealthStatus
       ...
       --   in Seal.Infra.Diffusion.Types, above 'healthStatus' record field).
    -> Maybe EkgNodeMetrics
    -> Logic IO
...
diffusionLayerFullExposeInternals fdconf
    ...
    diffusionHealth <- case mEkgNodeMetrics of
                            ...
                            Just m  -> liftIO $ do
                                wqgM <- Monitoring.createGauge (withSealNamespace "diffusion.WriteQueue") $ enmStore m
                                wM   <- Monitoring.createGauge (withSealNamespace "diffusion.Window")     $ enmStore m
    ...
    let -- VerInfo is a diffusion-layer-specific thing. It's only used for
        ...
        -- copied from Seal.Worker (allWorkers). Each one was manually
        ...
        -- requestTipOuts from Seal.Network.Block.Types
        ...
        -- TODO put these into a Seal.Diffusion.Full.Ssc module.
