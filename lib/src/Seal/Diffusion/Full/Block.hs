...
module Seal.Diffusion.Full.Block
    ( getBlocks
    , requestTip
    , announceBlockHeader
...
import           Seal.Binary.Communication (serializeMsgSerializedBlock,
...
import           Seal.Chain.Update (BlockVersionData, bvdSlotDuration)
import           Seal.Communication.Limits (mlMsgBlock, mlMsgGetBlocks,
                     mlMsgGetHeaders, mlMsgHeaders, mlMsgStream,
                     mlMsgStreamBlock)
import           Seal.Core (ProtocolConstants (..), difficultyL)
...
import           Seal.DB (DBError (DBMalformed))
import           Seal.Infra.Communication.Listener (listenerConv)
import           Seal.Infra.Communication.Protocol (Conversation (..),
...
import           Seal.Infra.Diffusion.Types (DiffusionHealth (..))
import           Seal.Infra.Network.Types (Bucket)
import           Seal.Infra.Util.TimeWarp (nodeIdToAddress)
import           Seal.Logic.Types (Logic)
import qualified Seal.Logic.Types as Logic
import           Seal.Network.Block.Types (MsgBlock (..), MsgGetBlocks (..),
...
import           Seal.Util (_neHead, _neLast)
import           Seal.Util.Timer (Timer, startTimer)
import           Seal.Util.Trace (Severity (..), Trace, traceWith)
...
announceBlockHeader logTrace logic protocolConstants recoveryHeadersMessage enqueue header =  do
  ...
  where
    announceBlockDo nodeId = pure $ Conversation $ \cA -> do
        ...
        -- Copied from Seal.Chain.Security but made pure. The existing
