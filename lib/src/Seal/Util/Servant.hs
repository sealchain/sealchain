...
import           Seal.Infra.Util.LogSafe (BuildableSafe, SecuredText, buildSafe,
                     logInfoSP, plainOrSecureF, secretOnlyF)
import           Seal.Util.Wlog (LoggerName, LoggerNameBox, usingLoggerName)
...
instance HasClient m (subApi :> res) => HasClient m (CDecodeApiArg subApi :> res) where
    ...
    hoistClientMonad pm _ f =
      hoistClientMonad pm (Proxy @(subApi :> res)) f
...
instance (RunClient m, HasClient m (Verb mt st ct $ ApiModifiedRes mod a)) =>
    ...
    hoistClientMonad pm _ f =
      hoistClientMonad pm (Proxy @(Verb mt st ct $ ApiModifiedRes mod a)) f
...
instance (KnownSymbol sym, Flaggable flag, HasClient m api) => HasClient m (CustomQueryFlag sym flag :> api) where
    ...
    hoistClientMonad pm _ f req =
      hoistClientMonad pm (Proxy @(QueryFlag sym :> api)) f (req . fromBool) . toBool
