...
module Test.Seal.Configuration
       ( defaultTestConf
...
import           Seal.Chain.Block (HasBlockConfiguration, withBlockConfiguration)
import           Seal.Chain.Delegation (HasDlgConfiguration,
                     withDlgConfiguration)
import           Seal.Chain.Genesis as Genesis (Config (..),
                     GenesisProtocolConstants (..), GenesisSpec (..),
                     StaticConfig (..), mkConfig)
import           Seal.Chain.Ssc (HasSscConfiguration, withSscConfiguration)
import           Seal.Chain.Txp (TxpConfiguration (..))
import           Seal.Chain.Update (BlockVersionData, HasUpdateConfiguration,
                     withUpdateConfiguration)
import           Seal.Configuration (HasNodeConfiguration, withNodeConfiguration)
import           Seal.Crypto (ProtocolMagic)
import           Seal.Launcher.Configuration (Configuration (..),
                     HasConfigurations)
import           Seal.Util.Config (embedYamlConfigCT)
