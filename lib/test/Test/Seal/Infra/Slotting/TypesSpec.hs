...
module Test.Seal.Infra.Slotting.TypesSpec
    ( spec
    ) where
...
import           Seal.Core (EpochIndex (..), LocalSlotIndex (..), TimeDiff (..),
                     Timestamp (..))
import           Seal.Infra.Slotting.Types
...
-- stack test seal --fast --test-arguments "-m Test.Seal.Infra.Slotting.Types"
spec :: Spec
