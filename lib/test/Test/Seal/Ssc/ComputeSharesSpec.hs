...
-- | Specification of Seal.Chain.Ssc.computeSharesdistr

module Test.Seal.Ssc.ComputeSharesSpec
       ( spec
       ) where
...
import           Seal.Chain.Ssc (SharesDistribution, SscVerifyError,
                     computeSharesDistrPure, isDistrInaccuracyAcceptable,
                     sharesDistrMaxSumDistr)
import           Seal.Core (Coin, CoinPortion, StakeholderId, mkCoin,
                     unsafeAddressHash, unsafeCoinPortionFromDouble,
                     unsafeGetCoin, unsafeSubCoin)
import           Seal.Core.Common (applyCoinPortionDown, sumCoins)
import           Seal.DB.Lrc (RichmenType (..), findRichmenPure)
