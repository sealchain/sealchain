...
-- | Specification of Seal.Chain.Ssc

module Test.Seal.Ssc.Toss.BaseSpec
       ( spec
       ) where
...
import           Seal.Binary (AsBinary)
import           Seal.Chain.Lrc (RichmenStakes)
import           Seal.Chain.Ssc (Commitment, CommitmentSignature,
...
import           Seal.Core (Coin, EpochIndex, EpochOrSlot (..), StakeholderId,
                     addressHash, crucialSlot, mkCoin)
import           Seal.Crypto (DecShare, ProtocolMagic, PublicKey, SecretKey,
                     SignTag (SignCommitment), sign, toPublic)
import           Test.Seal.Chain.Lrc.Arbitrary (GenesisMpcThd,
                     ValidRichmenStakes (..))
import           Test.Seal.Util.QuickCheck.Property (qcElem, qcFail, qcIsRight)

import           Test.Seal.Chain.Genesis.Dummy (dummyBlockVersionData,
                     dummyConfig, dummyK)
import           Test.Seal.Chain.Ssc.Arbitrary (BadCommAndOpening (..),
...
checksBadCommsPayload
    ...
    let mrsWithMissingEpoch = HM.delete epoch mrs
        ...
        someRichman =
            case res of
                Just s -> s
                _ -> error "Test.Seal.Ssc.Toss.BaseSpec.checkBadCommsPayload:\
