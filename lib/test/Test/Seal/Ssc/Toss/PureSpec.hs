-- | Specification for Seal.Chain.Ssc.GodTossing.Toss.Pure

module Test.Seal.Ssc.Toss.PureSpec
       ( spec
       ) where
...
import           Seal.Chain.Ssc (InnerSharesMap, Opening, SignedCommitment,
                     VssCertificate (..))
import qualified Seal.Chain.Ssc as Toss
import           Seal.Core (EpochOrSlot, StakeholderId, addressHash)
