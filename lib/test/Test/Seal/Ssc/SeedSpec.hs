-- | Specification of Seal.calculateSeed

module Test.Seal.Ssc.SeedSpec
       ( spec
       ) where
...
import           Seal.Binary
import           Seal.Chain.Ssc (Commitment (..), CommitmentsMap, Opening (..),
                     SscSeedError (..), calculateSeed, getCommShares,
                     getCommitmentsMap, mkCommitmentsMap,
                     randCommitmentAndOpening, secretToSharedSeed,
                     vssThreshold)
import           Seal.Core (AddressHash, SharedSeed (..), StakeholderId,
                     addressHash, mkCoin)
import           Seal.Crypto (DecShare, PublicKey, SecretKey,
...
import           Test.Seal.Util.QuickCheck.Arbitrary (nonrepeating, sublistN)
...
spec = do
    ...
    -- Test.Seal.Configuration as well
