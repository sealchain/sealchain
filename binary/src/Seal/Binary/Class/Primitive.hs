...
module Seal.Binary.Class.Primitive
       ( serialize
       , serializeWith
       , serialize'
       , serializeBuilder
...
import           Seal.Binary.Class.Core (Bi (..), Size, apMono, cborError,
...
encodeCrcProtected x =
    E.encodeListLen 2 <> (encodeCrcProtectedRaw x)

encodeCrcProtectedRaw :: Bi a => a -> E.Encoding
encodeCrcProtectedRaw x =
    encodeUnknownCborDataItem body <> encode (crc32 body)
  where
    body = serialize x
